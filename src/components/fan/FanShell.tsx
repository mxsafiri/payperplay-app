...
export function FanShell({
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-background">
      ...
      <div className="fixed top-[-20%] right-[-10%] w-[500px] h-[500px] rounded-full bg-amber-500/[0.03] blur-[120px] pointer-events-none" />
      {/* Pink blob bottom-left */}
      <div className="fixed bottom-[-20%] left-[-10%] w-[400px] h-[400px] rounded-full bg-pink-500/[0.03] blur-[100px] pointer-events-none" />

      {/* Sidebar — desktop only */}
      <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-60 z-40 flex-col border-r border-border backdrop-blur-xl bg-background/90">
        {/* Logo */}
        <div className="p-4 border-b border-border">
          <Link href="/dashboard" className="flex items-center gap-2.5 group">
            ...
            <div>
              <span className="text-foreground font-mono font-bold text-sm tracking-widest uppercase italic -skew-x-6 inline-block">
                PayPerPlay
              </span>
            </div>
          </Link>
            ...
            return (
              <Link
                ...
                className={`flex-1 flex flex-col items-center justify-center gap-1 text-[9px] font-mono uppercase tracking-wider transition-colors ${
                  isActive ? "text-amber-500" : "text-foreground/30"
                }`}
              >
                <item.icon className={`w-4 h-4 transition-transform ${isActive ? "scale-110" : ""}`} />
                {item.label}
                {isActive && (
                  <span className="absolute bottom-0 w-6 h-0.5 bg-amber-500" />
                )}
              </Link>
            );
          })}
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3.5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h1 className="text-lg font-bold font-mono tracking-tight text-foreground">{title}</h1>
              </div>
